...
def LogisticRegressor(train_X, val_X, test_X, train_Y, val_Y, test_Y):

    train_X = np.concatenate((train_X, val_X), axis = 0)
    train_Y = np.concatenate((train_Y, val_Y), axis = 0)

    learn_model = LogisticRegression(random_state= None, max_iter=2000, n_jobs=-1)

    parameters = {'C' : range(1,301,25),'penalty' : ['l2', 'none', 'newton-cg']} #, 'solver' : ['lbfgs', 'liblinear'] }
    parameters = {}
